...
void SparseCholeskySolver::compute_vertices_index() {
  ...
  for(int i = 0; i < solve_order_.size(); i++){
    block_dim_ = problem_ptr_->vertex_id_map_vertex_ptr_[solve_order_[i]]->minimal_dimension();
    vertex_id_map_start_index_[solve_order_[i]] = problem_params_size_;
    vertex_id_map_order_[solve_order_[i]] = i; //store the order of this vertex
    problem_params_size_ += problem_ptr_->vertex_id_map_vertex_ptr_[solve_order_[i]]->minimal_dimension();
  }
  for(int i = 0; i < solve_order_.size(); i++){
    for(int j = i; j < solve_order_.size(); j++){
      ...
      int dim_c = problem_ptr_->vertex_id_map_vertex_ptr_[solve_order_[j]]->minimal_dimension();
      hessian_block_[i][j] = Block(r,c,dim_r, dim_c,false);
      if(i!=j){
        hessian_block_[j][i] = Block(c,r,dim_c, dim_r, false);
      }
    }
  }
}
...
void SparseCholeskySolver::build_solve_structure() {
  ...
  for (auto &edge: problem_ptr_->edge_id_map_edge_ptr_) {
    ...
    for (size_t i = 0; i < edge.second->num_vertices(); i++) {
      ...
      int order_i = vertex_id_map_order_[v_ptr_i->id()];
...
void SparseCholeskySolver::solve_delta_x() {
  ...
  vec_c.resize(problem_params_size_);
